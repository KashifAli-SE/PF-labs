...
window.geometry("400x500")  # Initial size
...
for (text, row, col) in buttons:
    if text == "=":
        btn = tk.Button(btns_frame, text=text,font=("Helvetica", 20, "bold"),fg="bLack", bg="white", cursor="hand2", command=btn_equal)
    else:
        btn = tk.Button(btns_frame, text=text,font=("Helvetica", 20, "bold"), fg="black", bg="blue", cursor="hand2", command=lambda t=text: btn_click(t))
...
for (text, row, col) in scientific_buttons:
    if text == "C":
        btn = tk.Button(btns_frame, text=text,font=("Helvetica", 20, "bold"), fg="black", bg="#f00", cursor="hand2", command=btn_clear)
    else:
        btn = tk.Button(btns_frame, text=text, font=("Helvetica", 20, "bold"),fg="black", bg="grey", cursor="hand2", command=lambda t=text: scientific_function(t))
    btn.grid(row=row, column=col, sticky="nsew", padx=1, pady=1)
